...
if "method" not in st.session_state:
    st.session_state.method = None        # "oauth2" 또는 "oidc"

CHALLENGE_SERVER = "https://a2g.samsungds.net:47777"
...
if st.session_state.user is None:
    ...
    st.info("바이브 코딩으로 아래 로그인 버튼에 SSO를 연동하세요.")

    # TODO: 바이브 코딩으로 이 버튼에 OAuth2/OIDC 로그인을 연결하세요
...
# ============================================
    ...
    method = st.session_state.method or "oauth2"

    st.success(f"✅ {method.upper()} 로그인 성공!")
    ...
    # 제출할 데이터 미리 표시
    challenge_id = "sso_oidc" if method == "oidc" else "sso_oauth2"
    submit_data = {
    ...
    if method == "oidc":
        submit_data["answer"]["method"] = "oidc"
    ...
    if st.button("🎯 Challenge 서버에 제출"):
        if not st.session_state.access_token:
            ...
            try:
                resp = requests.post(
                    f"{CHALLENGE_SERVER}/challenges/{challenge_id}/submit",
                    json=submit_data,
                    verify=False,
                    timeout=10,
                )
